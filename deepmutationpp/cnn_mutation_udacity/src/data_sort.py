...
def check_data(ori_model_path, mutants_path, x, y, save_path, dataset):
    ...
    orig_predictions = os.path.join('..', '..', "predictions","predictions_udacity", "orig", 'orig_' + dataset + '.npy')
    mut_predctions = os.path.join('..', '..', "predictions","predictions_udacity", 'mutated')

    if not os.path.exists(orig_predictions):
        ori_model = load_model(ori_model_path, compile=False)

        ori_predict = ori_model.predict(x).squeeze()#.argmax(axis=-1)
    ...
    for path in model_path:
        ...
        if not os.path.exists(mut_predctions_path):
            ...
            result = model.predict(x).squeeze()#.argmax(axis=-1)
